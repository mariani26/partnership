...
/** @var backend\models\TbIndustriEngSearch $searchModel */
/** @var yii\data\ActiveDataProvider $dataProvider */

$this->title = 'Industrys';
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="tb-industri-eng-index">

	<h1><?= Html::encode($this->title) ?></h1>

	<p>
		<!-- <?= Html::a('Create Tb Industri Eng', ['create'], ['class' => 'btn btn-success']) ?> -->
	...
	<!-- <?= GridView::widget([
				...
				'columns' => [
					...
					[
						...
						'urlCreator' => function ($action, TbIndustriEng $model, $key, $index, $column) {
				...
				<div class="card-header border-0 pt-6">
					...
					<div class="card-title">
						...
						<div class="d-flex align-items-center position-relative my-1">
							...
							<!--end::Svg Icon-->
							<input type="text" data-kt-customer-table-filter="search" class="form-control form-control-solid w-250px ps-15" placeholder="Search Industry" />
						</div>
						<!--end::Search-->
					</div>
					<!--begin::Card title-->
					...
					<div class="card-toolbar">
						...
						<div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
							<p>
								...
								<?= Html::a('Add Industry', ['signupeng'], ['class' => 'btn btn-light-primary']) ?>
							</p>
						</div>
						<!--end::Toolbar-->
						<!--begin::Group actions-->
					...
					<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table">
						...
						<thead>
							...
							<tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
								...
								<th class="">Industry Name</th>
								<th class="">Identity Number</th>
								<th class="">Director Identity</th>
								<th class="">Business Field</th>
						...
						<tbody class="fw-bold text-gray-600">
							<?php foreach ($tb_industri_eng as $a) { ?>
								<tr>
									<!--begin::Checkbox-->
									<td>
									...
									<td><?= $a->industry_name ?></td>
									<td><?= $a->identity_number ?></td>
									<td><?= $a->director_identity ?></td>
									<td><?= $a->business_fields ?></td>
									...
									<td>
										<?php if ($a->doc_mou) { ?>
											<a href="<?= Yii::getAlias('@web/uploads_industri_eng/mou/' . $a->doc_mou) ?>" id="viewdokmu" target="_blank"><?= $a->doc_mou ?></a>
										<?php } ?>
									</td>
									<td>
										<?php if ($a->doc_moa) { ?>
											<a href="<?= Yii::getAlias('@web/uploads_industri_eng/moa/' . $a->doc_moa) ?>" id="viewdokmu" target="_blank"><?= $a->doc_moa ?></a>
										<?php } ?>
									</td>
									<td>
										<?php if ($a->doc_imp) { ?>
											<a href="<?= Yii::getAlias('@web/uploads_industri_eng/imp/' . $a->doc_imp) ?>" id="viewdokmu" target="_blank"><?= $a->doc_imp ?></a>
										<?php } ?>
									</td>
									<!-- <td><?= $a->initiator ?></td>
													<td><?= $a->description ?></td> -->
									<td><?= $a->main_email ?></td>
									<td><?= $a->backup_email ?></td>
