...
use backend\models\TbNpoSearch;
...
/**
 * TbNpoController implements the CRUD actions for TbNpo model.
 */
class TbNpoController extends Controller
{
    /**
     * @inheritDoc
     */
    ...
    /**
     * Lists all TbNpo models.
     *
     * @return string
     */
    public function actionIndex()
    {
        $searchModel = new TbNpoSearch();
        $dataProvider = $searchModel->search($this->request->queryParams);
        $tb_npo = TbNpo::find()->all();
    ...
    public function actionSignup()
    {
        ...
        if ($modelNgo->load(Yii::$app->request->post()) && $modelNgo->signup_Ngo()) {
            $user = User::find()->where(['username' =>  $modelNgo->username])->one();
            Yii::$app->session->setFlash('success', 'Thank you for registration. Please check your inbox for verification email.');
    ...
    public function actionCreate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->dok_mou) {
                $model->dok_mou->saveAs('uploads_npo/mou/' . $model->dok_mou->baseName . '.' . $model->dok_mou->extension);
                $model->dok_mou = $model->dok_mou->baseName . '.' . $model->dok_mou->extension;
            }
            if ($model->dok_moa) {
                $model->dok_moa->saveAs('uploads_npo/moa/' . $model->dok_moa->baseName . '.' . $model->dok_moa->extension);
                $model->dok_moa = $model->dok_moa->baseName . '.' . $model->dok_moa->extension;
            }
            if ($model->dok_imp) {
                $model->dok_imp->saveAs('uploads_npo/imp/' . $model->dok_imp->baseName . '.' . $model->dok_imp->extension);
    ...
    /**
     * Updates an existing TbNpo model.
    ...
    /**
     * Deletes an existing TbNpo model.
    ...
    /**
     * Finds the TbNpo model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return TbNpo the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TbNpo::findOne(['id' => $id])) !== null) {
            return $model;
        }
        ...
        if (Yii::$app->request->post()) {
            ...
            if ($modelImport->fileImport && $modelImport->validate()) {
                ...
                while (!empty($sheetData[$baseRow]['B'])) {
                    $model = new TbNpo();
                    $model->nama_npo = (string)$sheetData[$baseRow]['B'];
                    $model->bidang_npo = (string)$sheetData[$baseRow]['C'];
    ...
    public function actionExportExcel2()
    {
        ...
        $template = Yii::getAlias('@hscstudio/export') . '/templates/opentbs/data-npo.xlsx';
        ...
        foreach ($dataProvider->getModels() as $dinas) {
            $data[] = [
                'no' => $no++,
                'nama_npo' => $dinas->nama_npo,
                'bidang_npo' => $dinas->bidang_npo,
        ...
        $OpenTBS->Show(OPENTBS_DOWNLOAD, 'data_npo.xlsx'); // Also merges all [onshow] automatic fields.			
        exit;
    }
}
