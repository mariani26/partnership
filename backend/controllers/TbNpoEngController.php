...
use backend\models\TbNpoEngSearch;
...
/**
 * TbNpoEngController implements the CRUD actions for TbNpoEng model.
 */
class TbNpoEngController extends Controller
{
    /**
     * @inheritDoc
     */
    ...
    /**
     * Lists all TbNpoEng models.
     *
     * @return string
     */
    public function actionIndex()
    {
        $searchModel = new TbNpoEngSearch();
        $dataProvider = $searchModel->search($this->request->queryParams);
        $tb_npo_eng = TbNpoEng::find()->all();

        return $this->render('index', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
            'tb_npo_eng' => $tb_npo_eng,
        ]);
    }

    /**
     * Displays a single TbNpoEng model.
     * @param int $id ID
     * @return string
    ...
    public function actionSignupeng()
    {
        ...
        $modelNpoEng = new SignupFormNgoEng();
        if ($modelNpoEng->load(Yii::$app->request->post()) && $modelNpoEng->signup_NGOeng()) {
            $user = User::find()->where(['username' =>  $modelNpoEng->username])->one();
            Yii::$app->session->setFlash('success', 'Thank you for registration. Please check your inbox for verification email.');
        ...
        return $this->render('signupeng', [
            'model' => $model,
            'modelNpoEng' => $modelNpoEng,

        ]);
    }

    /**
     * Creates a new TbNpoEng model.
    ...
    public function actionCreate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->doc_mou) {
                $model->doc_mou->saveAs('uploads_npo_eng/mou/' . $model->doc_mou->baseName . '.' . $model->doc_mou->extension);
                $model->doc_mou = $model->doc_mou->baseName . '.' . $model->doc_mou->extension;
            }
            if ($model->doc_moa) {
                $model->doc_moa->saveAs('uploads_npo_eng/moa/' . $model->doc_moa->baseName . '.' . $model->doc_moa->extension);
                $model->doc_moa = $model->doc_moa->baseName . '.' . $model->doc_moa->extension;
            }
            if ($model->doc_imp) {
                $model->doc_imp->saveAs('uploads_npo_eng/imp/' . $model->doc_imp->baseName . '.' . $model->doc_imp->extension);
    ...
    /**
     * Updates an existing TbNpoEng model.
    ...
    /**
     * Deletes an existing TbNpoEng model.
    ...
    /**
     * Finds the TbNpoEng model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return TbNpoEng the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TbNpoEng::findOne(['id' => $id])) !== null) {
            return $model;
        }
        ...
        if (Yii::$app->request->post()) {
            ...
            if ($modelImport->fileImport && $modelImport->validate()) {
                ...
                while (!empty($sheetData[$baseRow]['B'])) {
                    $model = new TbNpoEng();
                    $model->npo_name = (string)$sheetData[$baseRow]['B'];
                    $model->npo_field = (string)$sheetData[$baseRow]['C'];
                    ...
                    $model->email = (string)$sheetData[$baseRow]['N'];
    ...
    public function actionExportExcel2()
    {
        ...
        $template = Yii::getAlias('@hscstudio/export') . '/templates/opentbs/npo-data.xlsx';
        ...
        foreach ($dataProvider->getModels() as $school) {
            $data[] = [
                'no' => $no++,
                'npo_name'               => $school->npo_name,
                'npo_field'               => $school->npo_field,
        ...
        $OpenTBS->Show(OPENTBS_DOWNLOAD, 'npo-data.xlsx'); // Also merges all [onshow] automatic fields.			
        exit;
    }
}
