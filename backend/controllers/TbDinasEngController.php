...
use backend\models\TbDinasEngSearch;
...
/**
 * TbDinasEngController implements the CRUD actions for TbDinasEng model.
 */
class TbDinasEngController extends Controller
{
    /**
     * @inheritDoc
     */
    ...
    /**
     * Lists all TbDinasEng models.
     *
     * @return string
     */
    public function actionIndex()
    {
        $searchModel = new TbDinasEngSearch();
        $dataProvider = $searchModel->search($this->request->queryParams);
        $tb_dinas_eng = TbDinasEng::find()->all();

        return $this->render('index', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
            'tb_dinas_eng' => $tb_dinas_eng,
        ]);
    }
    ...
    public function actionSignupeng()
    {
        ...
        if ($modelDinasEng->load(Yii::$app->request->post()) && $modelDinasEng->signup_dinaseng()) {
            $user = User::find()->where(['username' =>  $modelDinasEng->username])->one();
            Yii::$app->session->setFlash('success', 'Thank you for registration. Please check your inbox for verification email.');
    ...
    /**
     * Displays a single TbDinasEng model.
     * @param int $id ID
     * @return string
    ...
    /**
     * Creates a new TbDinasEng model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return string|\yii\web\Response
     */
    public function actionCreate($id)
    {
        $model = new TbDinasEng();
        $model->user_id = $id;
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->doc_mou) {
                $model->doc_mou->saveAs('uploads_dinas_eng/mou/' . $model->doc_mou->baseName . '.' . $model->doc_mou->extension);
                $model->doc_mou = $model->doc_mou->baseName . '.' . $model->doc_mou->extension;
            }
            if ($model->doc_moa) {
                $model->doc_moa->saveAs('uploads_dinas_eng/moa/' . $model->doc_moa->baseName . '.' . $model->doc_moa->extension);
                $model->doc_moa = $model->doc_moa->baseName . '.' . $model->doc_moa->extension;
            }
            if ($model->doc_imp) {
                $model->doc_imp->saveAs('uploads_dinas_eng/imp/' . $model->doc_imp->baseName . '.' . $model->doc_imp->extension);
    ...
    /**
     * Updates an existing TbDinasEng model.
    ...
    /**
     * Deletes an existing TbDinasEng model.
    ...
    /**
     * Finds the TbDinasEng model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return TbDinasEng the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TbDinasEng::findOne(['id' => $id])) !== null) {
            return $model;
        }
        ...
        if (Yii::$app->request->post()) {
            ...
            if ($modelImport->fileImport && $modelImport->validate()) {
                ...
                while (!empty($sheetData[$baseRow]['B'])) {
                    $model = new TbDinasEng();
                    $model->agency_name = (string)$sheetData[$baseRow]['B'];
                    $model->cooperation_field = (string)$sheetData[$baseRow]['C'];
    ...
    public function actionExportExcel2()
    {
        ...
        $template = Yii::getAlias('@hscstudio/export') . '/templates/opentbs/government-data.xlsx';
        ...
        $OpenTBS->Show(OPENTBS_DOWNLOAD, 'government-data.xlsx'); // Also merges all [onshow] automatic fields.			
        exit;
    }
}
