...
use backend\models\TbIndustriEngSearch;
...
class TbIndustriEngController extends Controller
{
    ...
    {
        $searchModel = new TbIndustriEngSearch();
    ...
    public function actionSignupeng()
    {
        ...
        if ($modelIndustriEng->load(Yii::$app->request->post()) && $modelIndustriEng->signup_industrieng()) {
            $user = User::find()->where(['username' =>  $modelIndustriEng->username])->one();
            Yii::$app->session->setFlash('success', 'Thank you for registration. Please check your inbox for verification email.');
    ...
    public function actionCreate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->doc_mou) {
                $model->doc_mou->saveAs('uploads_industri_eng/mou/' . $model->doc_mou->baseName . '.' . $model->doc_mou->extension);
                $model->doc_mou = $model->doc_mou->baseName . '.' . $model->doc_mou->extension;
            }
            if ($model->doc_moa) {
                $model->doc_moa->saveAs('uploads_industri_eng/moa/' . $model->doc_moa->baseName . '.' . $model->doc_moa->extension);
                $model->doc_moa = $model->doc_moa->baseName . '.' . $model->doc_moa->extension;
            }
            if ($model->doc_imp) {
                $model->doc_imp->saveAs('uploads_industri_eng/imp/' . $model->doc_imp->baseName . '.' . $model->doc_imp->extension);
        ...
        if (Yii::$app->request->post()) {
            ...
            if ($modelImport->fileImport && $modelImport->validate()) {
                ...
                while (!empty($sheetData[$baseRow]['B'])) {
                    $model = new TbIndustriEng();
                    $model->industry_name = (string)$sheetData[$baseRow]['B'];
                    $model->identity_number = (string)$sheetData[$baseRow]['C'];
                    $model->director_identity = (string)$sheetData[$baseRow]['D'];
                    $model->business_fields = (string)$sheetData[$baseRow]['E'];
                    $model->collaboration_title = (string)$sheetData[$baseRow]['F'];
                    $model->type_of_cooperation = (string)$sheetData[$baseRow]['G'];
                    $model->collaboration_year_number = (string)$sheetData[$baseRow]['H'];
                    $model->start_date = (string)$sheetData[$baseRow]['I'];
                    $model->end_date = (string)$sheetData[$baseRow]['J'];
                    $model->doc_mou = (string)$sheetData[$baseRow]['K'];
                    $model->doc_moa = (string)$sheetData[$baseRow]['L'];
                    $model->doc_imp = (string)$sheetData[$baseRow]['M'];
                    $model->initiator = (string)$sheetData[$baseRow]['N'];
                    $model->description = (string)$sheetData[$baseRow]['O'];
                    $model->main_email = (string)$sheetData[$baseRow]['P'];
                    $model->backup_email = (string)$sheetData[$baseRow]['Q'];
                    $model->phone_number = (string)$sheetData[$baseRow]['R'];
                    $model->fax_number = (string)$sheetData[$baseRow]['S'];
                    $model->website = (string)$sheetData[$baseRow]['T'];
                    $model->address = (string)$sheetData[$baseRow]['U'];
                    $model->city = (string)$sheetData[$baseRow]['V'];
                    $model->state = (string)$sheetData[$baseRow]['W'];
                    $model->country = (string)$sheetData[$baseRow]['X'];
                    $model->zipcode = (string)$sheetData[$baseRow]['Y'];
                    $model->contact_person = (string)$sheetData[$baseRow]['Z'];
                    $model->map_link = (string)$sheetData[$baseRow]['AA'];
                    $model->facebook_link = (string)$sheetData[$baseRow]['AB'];
                    $model->instagram_link = (string)$sheetData[$baseRow]['AC'];
                    $model->twitter_link = (string)$sheetData[$baseRow]['AD'];
                    $model->youtube_link = (string)$sheetData[$baseRow]['AE'];
                    $model->user_id = (string)$sheetData[$baseRow]['AF'];
                    $model->save();
    ...
    public function actionExportExcel2()
    {
        ...
        $template = Yii::getAlias('@hscstudio/export') . '/templates/opentbs/data-industry.xlsx';
        ...
        foreach ($dataProvider->getModels() as $industri) {
            $data[] = [
                'no' => $no++,
                'industry_name'             => $industri->industry_name,
                'identity_number'           => $industri->identity_number,
                'director_identity'         => $industri->director_identity,
                'business_fields'           => $industri->business_fields,
                ...
                'backup_email'              => $industri->backup_email,
        ...
        $OpenTBS->Show(OPENTBS_DOWNLOAD, 'data-industry.xlsx'); // Also merges all [onshow] automatic fields.			
        exit;
    }
}
