...
use backend\models\TbOrganisasiSearch;
...
/**
 * TbOrganisasiController implements the CRUD actions for TbOrganisasi model.
 */
class TbOrganisasiController extends Controller
{
    /**
     * @inheritDoc
     */
    ...
    /**
     * Lists all TbOrganisasi models.
     *
     * @return string
     */
    public function actionIndex()
    {
        $searchModel = new TbOrganisasiSearch();
        $dataProvider = $searchModel->search($this->request->queryParams);
        $tb_organisasi = TbOrganisasi::find()->all();


        return $this->render('index', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
            'tb_organisasi' => $tb_organisasi,
        ]);
    }

    /**
     * Displays a single TbOrganisasi model.
     * @param int $id ID
     * @return string
    ...
    public function actionSignup()
    {
        ...
        if ($modelOrganisasi->load(Yii::$app->request->post()) && $modelOrganisasi->signup_organisasi()) {
            $user = User::find()->where(['username' =>  $modelOrganisasi->username])->one();
            Yii::$app->session->setFlash('success', 'Thank you for registration. Please check your inbox for verification email.');
    ...
    /**
     * Creates a new TbOrganisasi model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return string|\yii\web\Response
     */
    public function actionCreate($id)
    {
        $model = new TbOrganisasi();
        $model->id_user = $id;
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->dok_mou) {
                $model->dok_mou->saveAs('uploads_organisasi/mou/' . $model->dok_mou->baseName . '.' . $model->dok_mou->extension);
                $model->dok_mou = $model->dok_mou->baseName . '.' . $model->dok_mou->extension;
            }
            if ($model->dok_moa) {
                $model->dok_moa->saveAs('uploads_organisasi/moa/' . $model->dok_moa->baseName . '.' . $model->dok_moa->extension);
                $model->dok_moa = $model->dok_moa->baseName . '.' . $model->dok_moa->extension;
            }
            if ($model->dok_imp) {
                $model->dok_imp->saveAs('uploads_organisasi/imp/' . $model->dok_imp->baseName . '.' . $model->dok_imp->extension);
    ...
    /**
     * Updates an existing TbOrganisasi model.
    ...
    /**
     * Deletes an existing TbOrganisasi model.
    ...
    /**
     * Finds the TbOrganisasi model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return TbOrganisasi the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TbOrganisasi::findOne(['id' => $id])) !== null) {
            return $model;
        }
        ...
        if (Yii::$app->request->post()) {
            ...
            if ($modelImport->fileImport && $modelImport->validate()) {
                ...
                while (!empty($sheetData[$baseRow]['B'])) {
                    $model = new TbOrganisasi();
                    $model->nama_organisasi = (string)$sheetData[$baseRow]['B'];
                    $model->bidang_organisasi = (string)$sheetData[$baseRow]['C'];
    ...
    public function actionExportExcel2()
    {
        ...
        $template = Yii::getAlias('@hscstudio/export') . '/templates/opentbs/data-organisasi.xlsx';
        ...
        foreach ($dataProvider->getModels() as $dinas) {
            $data[] = [
                'no' => $no++,
                'nama_organisasi' => $dinas->nama_organisasi,
                'bidang_organisasi' => $dinas->bidang_organisasi,
        ...
        $OpenTBS->Show(OPENTBS_DOWNLOAD, 'data_organisasi.xlsx'); // Also merges all [onshow] automatic fields.			
        exit;
    }
}
